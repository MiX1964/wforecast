...
class maps:
    ...
    def addcircle(self, lat,lng,rad,color):
        self.circles.append((lat,lng,rad,color))
    ...
    def draw(self, htmlfile):
        ...
        f.write('<title>Google Maps - pygmaps by XL </title>\n')
        ...
        f.write('\t<div id="map_canvas" style="width: 720px; height: 400px;"></div>\n')
        f.write('</body>\n')
        f.write('</html>\n')		
        f.close()
    ...
    def drawcircles(self, f):
        for circle in self.circles:
            self.drawcircle(f,circle[0],circle[1],circle[2],circle[3])
    ...
    def drawpoint(self,f,lat,lon, icon):
        icon_list = {'house':'house', 'station':'station', 'default':'red-circle'}
        if icon in icon_list:
            img = icon_list[icon]
        else:
            img = icon_list['default']
        f.write('\t\tvar latlng = new google.maps.LatLng(%f, %f);\n'%(lat,lon))
        f.write('\t\tvar img = new google.maps.MarkerImage("static/Google Maps Markers/%s.png");\n' % (img))
    ...
    def drawcircle(self, f, lat, lon, radius, color):
        f.write('\t\tvar latlng = new google.maps.LatLng(%f, %f);\n'%(lat,lon))
        f.write('\t\tvar marker = new google.maps.Circle({\n')
        f.write('\t\tfillColor: "%s",' % (color))
        f.write('\t\tfillOpacity: 0.2,')
        f.write('\t\tstrokeColor: "%s",' % (color))
        f.write('\t\tstrokeOpacity: 0.7,')
        f.write('\t\tstrokeWeight: 1,')
        f.write('\t\tcenter: latlng,\n')
        f.write('\t\tradius: %f\n' % (radius))
        f.write('\t\t});\n')
        f.write('\t\tmarker.setMap(map);\n')
        f.write('\n')                                  
...
if __name__ == "__main__":
	...
	# DESC:		add a point into a map and dispaly it
...
#	mymap.addradpoint(37.429, -122.145, 95, "#FF0000")
	...
	########## FUNCTION:  addcircle(latitude, longitude, radius, color)######################
	# DESC: 	add a shaded circle with a radius (Meter) - Draw circle
	# PARAMETER1:	latitude (float) latitude of the point
	# PARAMETER2:	longitude (float) longitude of the point
	# PARAMETER3:	radius (float), radius  in meter 
	# PARAMETER4:	color (string) color of the point showed in map, using HTML color code
	...
	# RETURN:	no return 
	#========================================================================================
    mymap.addcircle(37.429, -122.145, 95,"#0000FF")
